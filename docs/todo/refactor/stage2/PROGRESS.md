...
| 4 | [030 frontend hygiene](phase4-frontend-split/030-frontend-hygiene.md) | no | **silent-except baseline 38 → 0 (2026-08-31)** — every remaining swallow converted to a logged handler; the gate is now a rule, not a ratchet, with a planted-violation control. No page over 800 lines. **Timer premise measured and corrected**: 23 of 36 callbacks were already async and 9 more only touch their own labels; the real problem was **4** sync callbacks reaching a controller — all fixed, gate at zero. `components/poll.py` remains, on tidiness grounds only | Claude (for Darren) | NiceGUI canary landed (patches verified on 3.15); silent-except AST gate at a shrinking baseline 44→40 (4 header-refresh swallows now logged, incl. the whole-header one); timer→poll-helper migration NOT done (33 timers remain — follow-up) |
