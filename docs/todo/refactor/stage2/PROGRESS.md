...
- Phase 4 (frontend split): **effectively done (2026-09-02)** — contract 59→2, all splits landed, hygiene done. One item left: restructure 060 (enforce at zero), blocked on a decision, not effort
...
| 4 | [010 drive the restructure pack](phase4-frontend-split/010-drive-restructure.md) | no | **effectively done (2026-09-02)** | Claude (for Darren) | Contract 59 → **2**. Restructure phase-1 lanes 020/030/040/050 are all **obsolete** (audited 2026-09-01: every import in their Problem tables is gone) and phase-2 splits landed — no frontend file over 800 lines. The only phase-1 item left is [060 enforce at zero](../frontend/restructure/phase1-controller-boundary/060-enforce-at-zero.md), which is **blocked on a decision, not effort**: the last 2 sites are `frontend/app/__init__.py:70` (the composition root, already a sanctioned site in CLAUDE.md) and `frontend/pages/settings/_email.py:203`. Either name the composition-root exception in the contract and enforce at zero, or give both sites a controller. |
