...
| 1 | [060 enforce at zero](phase1-controller-boundary/060-enforce-at-zero.md) | no | **done (2026-09-02)** | Claude | Owner's decision: give `_email.py` a controller and enforce at zero. `notifications_controller.build_orb_report()` added; `_header.py` (the real second site, not `__init__.py` as this table said) now has `get_engine`/`ADMIN_AVAILABLE`/`admin_open_fn` injected from the composition root. `Contract` gained `exempt_files`, holding `frontend/app/__init__.py` alone, guarded by tests that require it to exist AND still violate — a stale exemption fails the build. |
