...
(`github.com/MooreSi/forex`), currently on branch `claude/refactor-plan-docs-pjn1hl`, and
...
**Who decides:** The developer (dev/ops).
...
## Open sub-questions for the developer
...
2. **Activate CI** — add `.github/workflows/checks.yml` and require it before merge to the default
   ...
   **Answer:** workflow file exists (stage1 4/010). Still open: **push the branch** (activates CI)
   and set branch protection — the developer's call, blocks the give-to-simon checklist's "CI green" row.
