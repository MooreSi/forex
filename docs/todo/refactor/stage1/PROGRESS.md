...
| 1 | [050 bind-localhost](phase1-stop-the-bleeding/050-bind-dashboard-localhost.md) | no | done (2026-08-10) | Claude | dashboard binds 127.0.0.1 by default; `host` config key + non-loopback warning; 7 new tests green |
| 2 | [010 gates-fail-closed](phase2-safety-net/010-guardrail-gates-fail-closed.md) | no | done (2026-08-10) | Claude | coverage fed; pyproject fixed; NEW module-reachability orphan gate (fails closed) replaces vacuous one; structure+import gates hardened to fail closed. 25 new tests. Green means green now. |
...
| 2 | [040 record-close-idempotency](phase2-safety-net/040-record-close-idempotency.md) | YES | **code done (2026-08-29)** — awaiting demo. **The PARTIAL close has the same shape and is NOT fixed** (found 2026-09-01) — its status guard cannot see a repeat because a partial leaves the trade open. Demonstrated by `tests/trading/test_partial_close_idempotency.py`; needs an owner decision first, see [simon-handover/018](../../../simon-handover/018-the-partial-close-can-pay-twice.md) | Claude | Compare-and-set on `apply_full_close`: only an `open` trade may close. Was double-crediting the balance on a duplicate call (`balance = balance + ?` with no guard). 10 tests, 4 mutants killed. |
...
| 2 | [060 news-calendar-offload](phase2-safety-net/060-news-calendar-offload.md) | no | done (2026-08-10) | Claude | getter now a pure cache read (no ~10s inline urllib); background refresher started at boot; None result cached (fixed every-cycle refetch); fetch+decision logic byte-identical. 6 new tests. Q004 policy (trade-through) unchanged. Full suite green. |
| 2 | [070 update-channel-disable](phase2-safety-net/070-update-channel-disable.md) | no | done (2026-08-10) | Claude | already off by default; added `_remote_client_enabled` regression guard + loud unauthenticated-warning on opt-in + honest update-panel banner. 5 new tests. Gates+boot green. |
| 3 | [010 delete-dead-code](phase3-expansion-tax/010-delete-dead-code.md) | no | partial (3 clones deleted; 4 unwired await Simon) | Claude | Deleted the 3 superseded-dead database.py clones + 6 files/3384 LOC (fixed 3 entangled tests — their "still needed" comments were STALE, verified empirically). 4 built-but-UNWIRED modules remain, ledgered, awaiting Simon's wire-vs-remove call (Q002). |
| 3 | [020 engine-shared-code](phase3-expansion-tax/020-consolidate-engine-shared-code.md) | no | **premise corrected + level question answered** (2026-08-31); indicator move still open | Claude | `test_signal` is the **Bounce** engine, not a test engine — dependency is production→production. Level detection is **not** three drifted copies: three different algorithms, pinned by `tests/refactor/test_level_detection_is_not_duplicated.py` (8 mutants killed). Step 3 of the task is void as written; choosing a survivor is Simon + a demo, not a refactor. **Indicator move deliberately NOT done (2026-08-31)** — reasons in the task file: it fixes no bug, and the eight movable functions straddle both halves of a mutually-dependent pair, the exact structure that produced bugs/018. A one-sitting job with the suite in front of you; the three gates that de-risk it now exist. Remaining scope is 8 functions, not ten: `identify_key_levels` and `is_news_window` should stay. Was: move the ten shared indicators to a shared home (pure move, fixes no bug) |
...
| 4 | [010 ci-job](phase4-hygiene/010-ci-job.md) | no | done — activates on push (2026-08-10) | Claude | repo+remote+gitignore already exist (Q003 corrected). `.github/workflows/checks.yml` runs `tools.checks all` on Windows on push/PR + guard tests. Red/green verification happens on first push. |
...
- All six QUESTIONS.md answers → recommendations adopted **provisionally**; final decision-maker is Simon, overrides possible until each consuming task is implemented (source: user, 2026-08-10)
...
- **NEW owner decision needed (phase3/010):** 4 of the 7 unreached modules are built-but-unwired
  ...
  a dev-vs-owner split either (licence/client is security/business). See orphan_module_allowlist.json.
