...
decision with a date and a rationale, not a rule to follow. `docs/history/` is an audit trail and
must not be edited.

## What must NOT change

- **`docs/history/` is not edited.** Not to correct it, not to append to it. It is an audit trail.
...
- After this task, `/spec done` verifies, harvests anything still missing, and deletes
  `docs/todo/frontend/restructure/`. Check nothing needed survives only in PROGRESS.md first.
