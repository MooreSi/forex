...
**Domain:** review-august-11 (release-readiness roadmap)
**Touches money:** YES — phase 5 (one task) and all of phase 6. `/safe-change` governs those; Simon's sign-off + a demo session are required before they are Done.
**Created:** 2026-08-11
...
5. **It's safe with real money** — the order-dedup / reconciliation / halts work is done and
   Simon-signed-off on a demo terminal. (Phase 6.)
...
- The frozen close path (`close_trade`, `record_close`, `_make_close_trade_ctx`,
  `partial_close_trade`) — called, never reshaped. See phase 6 and each money task.
...
| [phase6-money-path/](phase6-money-path/README.md) | Live-safety fixes — Simon's demo session |
...
| Pack | Relationship |
|---|---|
| [../review-august-08/](../review-august-08/README.md) | The Aug-8 remediation, mostly done. Phase 6 here = its phase-1 money tasks; phase 4 here folds in its phase-3 frontend tasks. |
...
answering the restructure QUESTIONS. Phase 5 makes debug mode fully usable. Phase 6 is gated on
**Simon** (sign-off + demo). Phase 7 closes out. Recommended first move: **phase 1 (usability)** —
it is the owner's stated top pain and is pure view-layer, no money.
...
- Money-path defaults already provisionally answered in review-august-08 QUESTIONS (Simon confirms)

## Out of scope

- New trading strategies / signal logic — this roadmap is usability, safety and maintainability only.
- Multi-user auth, cloud/remote hardening, cluster work — single-node localhost (see review-august-08).
