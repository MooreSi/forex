...
# Signal Climber: progressive exit fractions indexed by signal TP count.
# Last TP always closes all remaining lots regardless of fraction.
...
# Back-loaded ladder indexed by signal TP count (mirrors _CLIMBER_PCTS shape).
_GDVR_SL_MULT       = 4.0
...
class SimulationEngine:
    ...
    async def open_trade(self, signal_id: str, direction: str, entry_low: float, entry_high: float,
        ...
        if await db_module.to_db_thread(self.is_trading_paused):
        ...
        # whichever node's table is about to receive the INSERT. The forwarding
        # branch above returns before this point, so a forwarding Mac never
        # runs this against its own (largely empty, since forwarded trades
        # never land locally) table — it's the receiving node's own open_trade()
        # call, whether local or via a forwarded signal_order, that enforces it
        # against the count that's actually authoritative. Previously this
        # check lived only in open_trade_from_signal()/open_manual_market_order()
        # before they called this function — which meant a forwarding node's
        # check never saw the trades it had already forwarded, and the VPS's
        # _handle_signal_order (which calls open_trade() directly) had no
        # check of its own at all, so the cap was unenforced end-to-end
        # whenever centralized signal generation was on.
        ...
        if managed_by != "ea":
            mt5_result  = await self._bridge.place_order(direction, lot_size, stop_loss, mt5_tp,
                                                          f"sig:{signal_id[:8]}")
    ...
    async def partial_close_trade(self, trade_id: str, lots_to_close: float,
        ...
        partial_pnl   = self.pnl(direction, entry_price, close_price, lots_to_close)
    ...
    async def _handle_conservative(self, trade: dict, tick: Tick) -> None:
        ...
        _TRAIL_DIST = _CONSERVATIVE_TRAIL_PT   # trailing stop distance after TP1
        _close_pct  = 0.80
        ...
        if 1 not in triggered:
            ...
            self._log_tp_wait_diagnostic(trade_id, "conservative", direction, _cur_px, tp1_f, tp1_hit)
            if not tp1_hit:
                return
            ...
            try:
                res = await self.partial_close_trade(trade_id, lots_to_close, actual_price, "TP1")
                triggered.add(1)
                asyncio.create_task(telegram_alerts.send_message(
    ...
    async def _handle_scalp_runner(self, trade: dict, tick: Tick) -> None:
        ...
        if 1 not in triggered:
            ...
            tp1_hit = (direction == "BUY"  and tick.bid >= tp1_f) or \
                      (direction == "SELL" and tick.ask <= tp1_f)
            self._log_tp_wait_diagnostic(trade_id, "scalp_runner", direction, _cur_px, tp1_f, tp1_hit)
            if not tp1_hit:
                return
        ...
        if 2 not in triggered:
            ...
            self._log_tp_wait_diagnostic(trade_id, "scalp_runner", direction, _cur_px, tp2_f, tp2_hit)
            if not tp2_hit:
                return
                ...
                if remaining > 0:
                    ...
                    if mt5_ticket:
                        ...
                        if mt5_res.get("success"):
                            ...
                            return
                    try:
                        res = await self.partial_close_trade(trade_id, remaining, actual_price, "TP6")
                        triggered.add(6)
                        asyncio.create_task(telegram_alerts.send_message(
                        ...
                        if res.get("auto_closed") and mt5_ticket:
                            asyncio.create_task(
                                self._close_full_after_tps(trade_id, mt5_ticket, actual_price)
                            )
                    except Exception as exc:
        ...
        async def _close_all(tp_num: int, tp_val: float) -> None:
            ...
            if mt5_ticket:
                ...
                if mt5_res.get("success"):
                    ...
                    return
            try:
                res = await self.partial_close_trade(trade_id, remaining, actual_price, f"TP{tp_num}")
                triggered.add(tp_num)
                asyncio.create_task(telegram_alerts.send_message(
    ...
    async def _handle_dynamic_position_management(self, trade: dict, tick: Tick) -> None:
        ...
        """
        calibrated  = await db_module.to_db_thread(self._load_dpm_calibrated)
        triggered   = await self._get_triggered_tps(trade["trade_id"])
        ...
        params      = dpm_engine.compute_adaptive_params(
            trade, tick, self._dpm_candles,
            calibrated=calibrated,
            dxy_candles=self._dpm_dxy_candles,
        ...
        await db_module.to_db_thread(self._record_dpm_entry, trade, params)
        ...
        unrealized = self.pnl(direction, entry_price, current, float(trade["remaining_lots"]))

        # Update high-water P&L for calibration
        await db_module.to_db_thread(self._update_dpm_peak, trade_id, unrealized)
        ...
        if 1 not in triggered and tp1_val is not None:
            ...
            if tp1_cleared:
                ...
                await db_module.to_db_thread(self._set_dpm_milestone, trade_id, "reached_tp1")
                if not sl_at_be:
                    should_move = (direction == "BUY" and entry_price > current_sl) or \
                                  (direction == "SELL" and entry_price < current_sl)
                    if should_move:
        ...
        if not sl_at_be and unrealized >= be_trigger:
            ...
            if should_move:
                ...
                await db_module.to_db_thread(self._set_dpm_milestone, trade_id, "reached_be")
    ...
    async def _sync_profit(self, trade_id: str, mt5_ticket: int) -> Optional[float]:
        deals = await self._bridge.get_position_history(mt5_ticket)
        if not deals:
            all_deals = await self._bridge.get_deal_history(90)
    ...
    async def _schedule_profit_sync(self, trade_id: str, mt5_ticket: int) -> None:
        ...
        for delay in (0, 10, 60, 300, 1800):
            ...
            if row and row[0] is not None:
                return
            result = await self._sync_profit(trade_id, mt5_ticket)
    ...
    async def _profit_sweep(self) -> None:
        ...
        for row in rows:
            try:
                await self._sync_profit(row[0], int(row[1]))
            except Exception as e:
    ...
    async def _process_instant_entry(
        ...
        if bool(rs.get("risk_governor_enabled", 0)):
            ...
            if self._dpm_candles:
                try:
                    from forex_trader.core import dpm_engine as _dpm_rg2
                    _rg_atr_ime = _dpm_rg2.compute_atr(self._dpm_candles) or 0.0
                except Exception:
                    _rg_atr_ime = 0.0
            ...
            if strategy_lot > 0:
                ...
                _rg_bal_ime = await self._get_trading_balance()
            ...
            if self._dpm_candles:
                try:
                    from forex_trader.core import dpm_engine as _dpm_rp
                    _ime_atr_rp = _dpm_rp.compute_atr(self._dpm_candles) or 0.0
                except Exception:
                    _ime_atr_rp = 0.0
            _IME_SL_DIST = max(8.0, min(round(_ime_atr_rp * 1.2 if _ime_atr_rp > 0 else 12.0, 2), 25.0))
            _ime_bal_rp  = await self._get_trading_balance()
        ...
        try:
            ...
            if strategy in (STRATEGY_CONSERVATIVE, STRATEGY_SCALP_RUNNER) and trade_result.get("trade_id"):
                ...
                if _ime_tkt:
                    try:
                        await self._bridge.modify_order(int(_ime_tkt), sl=_ime_ex_sl, tp=None)
                    except Exception as _e:
            ...
            # ── Conservative Trial: set SL + 6 TPs immediately after fill ────
                ...
                try:
                    ...
                    if _cot_mt5:
                        try:
                            await self._bridge.modify_order(int(_cot_mt5), sl=exact_sl, tp=None)
    ...
    async def _apply_followup_to_instant_trade(
        ...
        if signal_id:
            await self.update_signal(signal_id, updates)
        else:
            ...
            if mt5_ticket:
                            ...
                            if _raw is not None:
                                ...
                                if (_dir == "BUY" and _tp_f > _entry) or \
                                   (_dir == "SELL" and _tp_f < _entry):
                                    safe_tp = _raw
                                break
                    await self._bridge.modify_order(
                ...
                # has no signal_id to route through that method, so it needs
    ...
    async def _find_and_apply_instant_followup(
        ...
        if not _instant or _instant.get("direction", "").upper() != direction.upper():
            return False
        await self._apply_followup_to_instant_trade(
            _instant, parsed, tg_id, channel_name, channel_name,
        )
        return True
    ...
    async def _ime_timeout_watchdog(self, tick: "Tick") -> None:
        ...
        for row in rows:
            ...
            _SELF_MGD = {STRATEGY_CONSERVATIVE, STRATEGY_CONSERVATIVE_TRIAL}
            if trade.get("strategy") in _SELF_MGD:
                log.warning(
    ...
    async def _tp_safety_net_sweep(self) -> None:
        ...
        for trade in open_trades:
            try:
                await self._tp_safety_net_check_trade(trade, now)
            except Exception as e:
    ...
    async def _tp_safety_net_check_trade(self, trade: dict, now: float) -> None:
        ...
        candles = await self._bridge.get_candles_range(open_time, now, "M1")
        if not candles:
            return
        if direction == "BUY":
            extreme = max(float(c.get("high", 0) or 0) for c in candles)
        ...
        if not entry_price:
            return
        cost_pts = self._compute_be_cost_pts(trade)
        ...
        trade_id = trade["trade_id"]
        last_alert = self._tp_safety_net_last_alert.get(trade_id, 0.0)
        cooldown_active = (now - last_alert) < self._TP_SAFETY_NET_ALERT_COOLDOWN
        ...
        if tick:
            ...
            if window_closed:
                if not cooldown_active:
                    self._tp_safety_net_last_alert[trade_id] = now
                    log.info(
        ...
        if mt5_ticket:
            try:
                mt5_res = await self._bridge.modify_order(int(mt5_ticket), sl=be_price, tp=None)
            except Exception as e:
    ...
    async def _cmd_help(self, args: list) -> str:
        return (
            ...
            "    `scale_out`  `be_runner`  `trail_stop`  `protected_scale`  `conservative`  `no_sl_scale`  `conservative_trial` (alias: `ct`)  `scalp_runner`  `signal_climber` (alias: `climber`)  `gd_vip_runner` (alias: `gdvr`)  `adaptive_runner` (alias: `adaptive`)\n"
    ...
    async def _cmd_daily(self, args: list) -> str:
        ...
        day_cutoff = _dt.now().replace(hour=0, minute=0, second=0, microsecond=0).timestamp()
        ...
        def _reason_lbl(exit_reason: str, trade_id: str = "") -> str:
            r = (exit_reason or "").lower()
            if _is_sl(r):
                ltp = self._last_closed_tp(trade_id) if trade_id else None
        ...
        if open_trades:
            ...
            for t in open_trades[:5]:
                ...
                pnl_t = (self.pnl(direction, entry_px,
    ...
    async def _cmd_status(self, args: list) -> str:
        ...
        tg_lines: list[str] = []
        if self._tg_reader is not None:
            tg_status = self._tg_reader.get_status()
    ...
    async def _cmd_strategy(self, args: list) -> str:
        ...
        if not strategy:
            return (
                ...
                "  `signal_climber`  `gd_vip_runner`  `adaptive_runner`"
            )
    ...
    async def _cmd_risk(self, args: list) -> str:
        ...
        try:
            ...
            if account and float(account.get("balance") or 0) > 0:
                ...
                bal = float(self.get_sim_account().get("balance", 1000))
    ...
    async def _cmd_activate(self, args: list) -> str:
        ...
        lot          = self.suggest_lot_size(entry_mid, tg_sig["stop_loss"], balance,
        ...
        result = await self.open_trade(
            signal_id=signal_id, direction=tg_sig["direction"],
